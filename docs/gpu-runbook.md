...
- 100 steps、prompt batch 2、group size 4，即名义 800 条在线 rollout；
...
脚本会重新安装固定 veRL commit、挂载当前项目与 Hugging Face cache、准备 veRL JSONL，并执行正式 preflight。以下任何条件都会在训练前失败：SFT handoff 哈希变化、训练数据过期、veRL 版本/commit 不符、镜像 digest 环境标记不符、无 CUDA/BF16、GPU 架构早于 Ampere。
...
- `configs/grpo.yaml`；
- 项目 Git SHA 与镜像 digest；
- 训练日志和每 25 步 checkpoint。

不要把模型、轨迹、日志或 checkpoint 提交到 Git。
