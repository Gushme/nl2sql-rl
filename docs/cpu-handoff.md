...
- CPU fixture 端到端演练已通过，覆盖只读双执行、错误后纠正、mock Teacher、统一评测 rollout、Action-only SFT、EX、SFT handoff、GRPO action mask 和同奖组动态过滤状态机。
...
动态过滤目前只有 CPU 分组/补采/上限测试和固定 veRL 补丁校验；真实过滤率、额外 rollout 数和训练收益必须等 GPU GRPO 运行后再报告。
